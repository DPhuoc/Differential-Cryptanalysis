...
def int_to_binary_list(n, bits=4):
...
def permutate_rev(block, table):
    res = [None for _ in range(len(block))]
...
def xor(block_1, block_2):
    return [block_1[i] ^ block_2[i] for i in range(len(block_1))]

def s_box(block):
    output = []
    for i in range(8):
        sub_str = block[i * 6:i * 6 + 6]
        row = 2 * sub_str[0] + sub_str[-1]
        column = binary_list_to_int(sub_str[1:5])
        output += int_to_binary_list(S_BOX_TABLE[i][row][column])
    return output

def one_s_box(block, sbox):
    row = 2 * block[0] + block[-1]
    column = binary_list_to_int(block[1:5])
    return int_to_binary_list(sbox[row][column])
...
def des(input_block, key, crypt_type):
